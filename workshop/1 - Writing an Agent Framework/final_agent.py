...
class BaseAgent(ABC):
    ...
    @overload
    def on(self, event: Literal["turn_start"], handler: TurnStartHook) -> "BaseAgent": ...

    @overload
    def on(self, event: Literal["llm_response"], handler: LLMResponseHook) -> "BaseAgent": ...

    @overload
    def on(self, event: Literal["llm_tool_call"], handler: LLMToolCallHook) -> "BaseAgent": ...
    ...
    def emit(self, event: HookType, *args: Any) -> None:
        if event == "verify_turn_complete":
            raise ValueError("Use verify_turn_complete() for verify_turn_complete hooks")
...
class Agent(BaseAgent):
    ...
    def get_contents(
        ...
        next_contents = [
            types.UserContent(parts=[types.Part.from_text(text=self.system_instruction)]),
            types.UserContent(parts=[types.Part.from_text(text=format_todos(run_state.todos))]),
            *contents,
        ]
    ...
    def update_run_state(
        ...
        function_calls = [part.function_call for part in message.parts or [] if part.function_call]
        if function_calls:
...
def print_llm_tool_result(
    ...
    if call.name == "modifyTodo":
        ...
        return
    if result.error:
        console.print(Text(f"  {call.name} failed: {result.response}", style="bold red"))
