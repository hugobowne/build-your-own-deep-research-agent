...
if __name__ == "__main__":
    ...
    async def on_ready() -> None:
        shell.print("Workshop 2 shell ready.")
        shell.update_region(
            "todos",
            "[bold]Todos[/bold]\n[ ] 1. Wire the main agent\n[ ] 2. Add steering\n[ ] 3. Add queueing",
        )
    ...
    async def on_submit(text: str) -> None:
        ...
        try:
            ...
            shell.print(
                f"[bold white]Assistant[/bold white]\nThe shell is live. Next step is wiring the agent runtime into it."
            )
        finally:
            shell.set_loading(False)

    shell.initialize(on_ready=on_ready, on_submit=on_submit)
    shell.run()
